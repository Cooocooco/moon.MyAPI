...
namespace moon.MyAPI.Controllers
{
    public class ModbusTCPController : ApiController
    {
        ...
        /// 异步读取设备的数据
        /// </summary>
        private async Task<Dictionary<string, object>> ReadRobotDataAsync(string ip, ModbusIpMaster adapter)
        {
        ...
        private async Task<DataModel> GetDataAsync(string ip)
        {
            ...
            using (var client = new TcpClient(ip, 502)) // 建立TCP连接
            {
                ...
                try
                {
                    var axisData = await ReadRobotDataAsync(ip, adapter).ConfigureAwait(false);
